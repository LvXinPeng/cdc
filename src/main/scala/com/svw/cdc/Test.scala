...
object Test {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val df = spark
      .readStream
      .format("kafka")
      .option("kafka.bootstrap.servers", "10.160.240.244:9092")
      .option("subscribe", "cdc.cdc.tb_cdc")
      .load()
    /*import spark.implicits._
    ...
    /*val operation = df.select("payload.op").first().getString(0)
    if (operation == "c" || operation == "u") {
      val cdcDF = df.select("payload.after.id", "payload.after.firstname", "payload.after.lastname", "payload.after.email")
        .toDF("id", "firstname", "lastname", "email")
      kuduContext.upsertRows(cdcDF, kudu_tb_name)

      val query = df.select("payload.after.id", "payload.after.firstname", "payload.after.lastname", "payload.after.email")
        .toDF("id", "firstname", "lastname", "email")
        .writeStream
        .format("console")
        .start()
